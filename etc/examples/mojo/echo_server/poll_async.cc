...
EventLoop::Event::Event(Client c, std::string& data) : _client(c), _data(data) {}
EventLoop::Event::Event(const Event& rhs) = default;
EventLoop::Event::~Event() = default;

EventLoop::Event& EventLoop::Event::operator=(const Event& h) {
  _client = h._client;
...
std::shared_ptr<Connection> accept_work(int sd, EventLoop* ev) {
  ...
  while (1) {
    ...
    // platform_handle.value = (uint64_t)cli_sd;
    // MojoHandle handle;
    // MojoResult result =
